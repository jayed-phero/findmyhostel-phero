...
const UpdateModal = ({ infodata, setInfoData, refetch }) => {
    ...
    const onSubmit = data => {
        ...
        const dayTwo = {
            oneAtten: data.twoAtten,
            oneMess: data.twoMess
        }
        const dayThree = {
...
{/* <div x-data="{ isOpen: true }" class="relative flex justify-center">
        ...
        //         </div>
        //         <div>
        //             <div onClick={() => setMess(!mess)} className={`px-5 py-1 text-sm  rounded-full dark:bg-gray-800 cursor-pointer ${mess ? 'text-indigo-500 bg-indigo-100/60' : 'text-pink-500 bg-pink-100/60'}`}>{mess ? "Yes" : "No"}</div>
        //         </div>
        //     </div>
        // </div>